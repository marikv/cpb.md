...
$file = $_REQUEST['file'] ?: '.';
$file = $file == '.' ? 'uploads' . DIRECTORY_SEPARATOR : $file;
if($_GET['do'] == 'list') {
    if (is_dir($file)) {
        $directory = $file;
        $result = [];
		...
		$(function(){
			...
			function renderFileRow(data) {
				function fileLink(path) {
					path = '/' + path;
					path = path.replace('//','/');
					return "javascript: alert('" + path + "');";
                }
				var $link = $('<a class="name" />')
					.attr('href', data.is_dir ? '#' + encodeURIComponent(data.path) : fileLink(data.path))
					.html('/uploads/' + data.name);
				...
				var $html = $('<tr />')
			        ...
			        .append( $('<td class="first" />').append('<a href="/uploads/'+ data.name + '" target="_blank"><img src="/uploads/'+ data.name + '" style="height: 40px;border-radius:5px;"/></a>') )
					.append( $('<td class="" />').append($link) )
...
<div id="top">
    ...
    <form action="?" method="post" id="mkdir" />
        {{csrf_field()}}
        <label for=dirname>Adaugă un folder nou</label><input id=dirname type=text name=name value="" />
        <input type="submit" value="Salvează" onclick="setTimeout(function () { document.location.reload(); }, 2000);" />
    </form>
